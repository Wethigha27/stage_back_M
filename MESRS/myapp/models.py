...
class User(AbstractUser):
    ...
    def get_service(self):
        ...
        if self.is_chef_service():
            try:
                return Service.objects.get(chef_service=self)
            except Service.DoesNotExist:
                return None
        
        # Si c'est un employé avec un profil
        if hasattr(self, 'personne') and self.personne:
            return self.personne.service
